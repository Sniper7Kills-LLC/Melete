...
cargo build --target wasm32-unknown-unknown -p journal-web-shim --release

echo "[build-wasm] cargo build --release --target wasm32-unknown-unknown -p journal-web-viewer"
cargo build --target wasm32-unknown-unknown -p journal-web-viewer --release
...
echo "  shim   → ${OUT_BASE}/shim/journal_web_shim.{js,wasm}"
echo "  viewer → ${OUT_BASE}/viewer/journal_web_viewer.{js,wasm}"
