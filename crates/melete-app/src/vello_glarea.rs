...
//! `MELETE_VELLO=1` is set at runtime. Off → returns None and window.rs
...
pub fn enabled() -> bool {
    ...
    // legacy `MELETE_VELLO=0` opt-out remains so the Cairo fallback can be
    // exercised for diagnostics; everything else defaults to on.
    !matches!(
        std::env::var("MELETE_VELLO").as_deref(),
...
pub fn build(state: SharedState) -> Option<GLArea> {
    if !enabled() {
        // MELETE_VELLO=0 → opt-out, fall back to the Cairo DrawingArea.
        return None;
    }
    ...
    let widgets_cell: Rc<RefCell<melete_widgets::WidgetRenderer>> =
        Rc::new(RefCell::new(melete_widgets::WidgetRenderer::new()));
    ...
    let widgets_cache: Rc<RefCell<Option<(u64, melete_widgets::VelloScene)>>> =
...

    {
        ...
        area.connect_render(move |area, _ctx| {
            ...
            let (
                ...
                let s = state.borrow();
                let mut frame: Vec<melete_core::Stroke> = s.strokes.clone();
                if let Some(cs) = s.current_stroke.clone() {
                    frame.push(cs);
                }
                let bg = melete_canvas::scale_background(&s.background, s.bg_scale);
                let widgets: Vec<melete_core::TemplateWidget> = s
                    .current_template
                    .as_ref()
                    .map(|t| t.widgets.clone())
                    .unwrap_or_default();
                let widget_ctx = melete_widgets::WidgetRenderContext {
                ...
                let selection_bbox = if s.selected_stroke_ids.is_empty() {
                    ...
                    melete_canvas::selection_combined_bbox(&s.strokes, &s.selected_stroke_ids)
                };
                ...
                let resolver: Box<dyn melete_core::AssetResolver> =
            ...
            let rgba = match vello_cell.borrow_mut().as_mut().unwrap().render_rgba(
                ...
                |scene, world_to_screen, pr| {
                    ...
                    let mut fresh = melete_widgets::VelloScene::new();
                    widgets_cell_inner.borrow_mut().draw_widgets(
                        &mut fresh,
                        melete_widgets::VelloAffine::IDENTITY,
                        &widgets,
                        pr,
                        &widget_ctx,
                    );
...
fn widget_cache_key(
    widgets: &[melete_core::TemplateWidget],
    ctx: &melete_widgets::WidgetRenderContext,
    page_rect: melete_core::Rect,
    dark_mode: bool,
) -> u64 {
...
fn bg_fingerprint<H: std::hash::Hasher>(bg: &melete_canvas::BackgroundConfig, h: &mut H) {
    use melete_canvas::BackgroundConfig as B;
    use std::hash::Hash;
    match bg {
        B::Blank => 0u8.hash(h),
