...
use melete_core::{NotebookId, NotebookTemplate, PageTemplate};
// NotebookStore methods reached via dyn NotebookBackend.
...
pub fn build(parent: &ApplicationWindow, state: SharedState) -> SharedWindow {
    ...
    let title_label = Label::new(Some("Melete"));
    ...
    {
        let win = win.clone();
        *tools_open.borrow_mut() = Some(Rc::new(move |seed: Option<melete_core::Brush>| {
    ...
    // user has opted in via app settings or `MELETE_DEV=1`. Shown
    ...
    if signed_in {
        ...
        signout_btn.add_css_class("destructive-action");
        {
            ...
            signout_btn.connect_clicked(move |_| {
                popover_clone.popdown();
                use melete_storage::remote_template_store::store::RemoteTemplateStore;
...
fn do_notebook_pdf_export(
    parent: &ApplicationWindow,
    state: SharedState,
    notebook_id: melete_core::NotebookId,
) {
    let dialog = gtk4::FileDialog::builder()
...
fn background_is_grid(bg: &melete_canvas::BackgroundConfig) -> bool {
