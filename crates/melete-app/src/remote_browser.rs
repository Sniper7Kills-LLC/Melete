...
use melete_storage::remote_template_store::store::{
...
pub type OpenPageTemplateEditorFn = std::rc::Rc<dyn Fn(melete_core::PageTemplate)>;
pub type OpenNotebookTemplateEditorFn = std::rc::Rc<dyn Fn(melete_core::NotebookTemplate)>;
pub type OpenBrushEditorFn = std::rc::Rc<dyn Fn(melete_core::Brush)>;
...
fn edit_into_local(kind: Kind, id: uuid::Uuid, state: SharedState) -> Result<(), RemoteError> {
    ...
    match kind {
        ...
        Kind::Brush => {
            ...
            let brush: melete_core::Brush = match toml::from_str(&row.body_toml) {
                Ok(b) => b,
                Err(e) => {
...
pub use melete_storage::remote_template_store::store::Visibility;
...
pub fn publish_local_page_template(
    template: &melete_core::PageTemplate,
    state: SharedState,
...
pub fn publish_local_notebook_template(
    template: &melete_core::NotebookTemplate,
    state: SharedState,
    visibility: Visibility,
) -> Result<(), RemoteError> {
    let mut store = RemoteTemplateStore::connect()?;
...
pub fn publish_local_brush(
    ...
    let row = melete_storage::BrushRow {
        id: brush.id,
