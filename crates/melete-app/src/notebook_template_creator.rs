...
use melete_canvas::{paint_with_widgets, ViewportTransform};
use melete_core::{
...
fn mini_page_preview(t: &PageTemplate) -> Frame {
    ...
    area.set_draw_func(move |_a, ctx, w, h| {
        ...
        let bg = melete_templates::page_template_to_background_config(&template);
...
pub fn build_editor_view(
    ...
    if read_only {
        ...
        let missing: Vec<TemplateId> = {
            ...
            referenced
                .into_iter()
                .filter(|id| reg.get(*id).is_none() && !melete_templates::is_builtin(*id))
                .collect()
        };
        if !missing.is_empty() {
...
fn swatch_color(bg: &melete_core::BackgroundType) -> (f64, f64, f64) {
    use melete_core::BackgroundType as BT;
    match bg {
