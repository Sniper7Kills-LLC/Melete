...
pub(crate) fn persist_notebook_template(
    state: &SharedState,
    t: &melete_core::NotebookTemplate,
) -> anyhow::Result<()> {
    let s = state.borrow();
...
pub fn prompt_new_page(
    ...
    on_ok: Box<
        dyn Fn(Option<TemplateId>, std::collections::HashMap<Uuid, melete_core::WidgetOverride>),
    >,
) {
    ...
    let row = build_button_row(&win, move || {
        ...
        let configurable: Vec<melete_core::TemplateWidget> = template
            .widgets
            .iter()
...
fn widget_is_configurable(kind: &melete_core::WidgetKind) -> bool {
    use melete_core::WidgetKind as K;
    matches!(
        kind,
        K::CalendarMonth
            | K::Timeline { .. }
...
fn prompt_widget_overrides(
    parent: &ApplicationWindow,
    widgets: Vec<melete_core::TemplateWidget>,
    on_ok: Box<dyn Fn(std::collections::HashMap<Uuid, melete_core::WidgetOverride>)>,
) {
    use gtk4::{Adjustment, ScrolledWindow, SpinButton};
    use melete_core::{WidgetKind as K, WidgetOverride as WO};
...
fn widget_kind_label(kind: &melete_core::WidgetKind) -> &'static str {
    use melete_core::WidgetKind as K;
    match kind {
...
pub fn prompt_notebook_template_editor(
    ...
    use melete_core::{DailySlot, NotebookTemplate, SectionTitleFormats};
