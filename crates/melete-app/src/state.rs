...
use gtk4::cairo;
use melete_canvas::{BackgroundConfig, GridSettings, ViewportTransform};
use melete_core::{
...
pub fn tool_brush_params(
    state: &CanvasState,
    tool: Tool,
) -> (f32, f64, melete_core::BlendMode, melete_core::ToolStyle) {
...
pub struct CanvasState {
    ...
    pub bg_scale: f64,
    pub page_rect: Rect,
    pub backend: Rc<RefCell<dyn NotebookBackend>>,
    ...
    pub brush_params: melete_canvas::vello_renderer::ToolStyleParams,
    ...
    pub active_brush_recipe: Option<melete_core::Brush>,
    ...
    pub brush_library: Vec<melete_core::Brush>,
    ...
    pub tool_brushes: std::collections::HashMap<String, melete_core::Brush>,
...
pub fn new_shared_state(
    ...
    let viewport = Viewport {
        center: melete_core::Point { x: 408.0, y: 528.0 },
        zoom: 1.0,
        rotation: 0.0,
    };
    ...
    let pen = PenSettings {
        ...
        base_width: 2.0,
        opacity: 1.0,
        blend_mode: melete_core::BlendMode::Normal,
        brush_style: melete_core::ToolStyle::Pen,
    };
    ...
    Rc::new(RefCell::new(CanvasState {
        ...
        brush_params: melete_canvas::vello_renderer::ToolStyleParams::default(),
        ...
        let (page_date, overrides, widget_data) = match b.get_page(page_id) {
            Ok(p) => {
                let d = match p.planner_address {
                    Some(melete_core::CalendarPageAddress::Day { date, .. }) => Some(date),
                    _ => None,
                };
...
pub fn set_current_template(state: &SharedState, template: Option<PageTemplate>) {
    ...
    match &template {
        Some(t) => {
            s.background = melete_templates::page_template_to_background_config(t);
            s.page_rect = Rect {
                x: 0.0,
                y: 0.0,
                width: t.size_mm.0,
