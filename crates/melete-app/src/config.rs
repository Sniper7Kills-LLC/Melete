...
pub struct AppConfig {
    ...
    /// `~/.config/journal/config.toml` or set the `MELETE_DEV=1`
    ...
    #[serde(default)]
    pub brush_params: Option<melete_canvas::vello_renderer::ToolStyleParams>,
...
/// `MELETE_DEV=1` environment opt-in for one-off debugging sessions.
pub fn developer_mode_enabled(cfg: &AppConfig) -> bool {
    if cfg.developer_mode {
        return true;
    }
    matches!(
        std::env::var("MELETE_DEV").as_deref(),
...
fn config_path() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("melete").join("config.toml"))
}
...
pub fn save(cfg: &AppConfig) -> std::io::Result<()> {
    ...
    // boot.
    melete_storage::fs_atomic::write_atomic(&p, text.as_bytes())
}
