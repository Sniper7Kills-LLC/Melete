...
use melete_core::{NotebookId, Page, PageId, Section, SectionId};
...
pub fn build_notebook_view(
    ...
    let is_planner = matches!(
        ...
        Ok(nb) if matches!(nb.kind, melete_core::NotebookKind::Planner { .. })
    );
...
fn load_page(state: &SharedState, page: &Page, canvas: &DrawingArea) {
    ...
    if let Some(melete_core::CalendarPageAddress::Day { date, .. }) = page.planner_address {
