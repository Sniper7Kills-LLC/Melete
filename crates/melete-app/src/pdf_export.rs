...
use melete_canvas::{paint, paint_with_widgets_ctx, ViewportTransform, WidgetRenderContext};
use melete_core::{NotebookId, Rect, SectionId, Viewport};
...
pub fn export_notebook_to_pdf(
    ...
    let mut ordered_page_ids: Vec<melete_core::PageId> = Vec::new();
    ...
    fn collect_pages_for_section(
        ...
        out: &mut Vec<melete_core::PageId>,
    ) -> anyhow::Result<()> {
    ...
    for page_id in ordered_page_ids {
        ...
        let (background, page_rect, widgets) = if let Some(ref t) = template_opt {
            let bg = melete_templates::page_template_to_background_config(t);
            let pr = Rect {
                x: 0.0,
                y: 0.0,
                width: t.size_mm.0,
