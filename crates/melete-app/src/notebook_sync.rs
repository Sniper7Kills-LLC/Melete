...
//! `melete_storage::remote_notebook_store` for the data shapes.
//!
...
fn paywall_from_sync_error(
    e: &melete_storage::remote_notebook_store::NotebookSyncError,
) -> Option<melete_storage::entitlement::PaywallError> {
    use melete_storage::remote_notebook_store::NotebookSyncError as N;
    use melete_storage::remote_template_store::graphql::GraphQlError;
    match e {
        N::GraphQl(GraphQlError::Service(msg)) => {
            melete_storage::entitlement::PaywallError::from_graphql_service_message(msg)
        }
        _ => None,
    }
}
...
/// on the main thread — touches the non-Send `dyn NotebookBackend`.
pub fn apply_pulled_strokes(
    state: &SharedState,
    notebook_id: NotebookId,
    pulled: Vec<melete_storage::remote_notebook_store::PulledStroke>,
) -> PullReport {
    let mut report = PullReport::default();
    let st = state.borrow();
    let mut backend = st.backend.borrow_mut();
...
mod queue {
    ...
    fn db_path() -> PathBuf {
        ...
        base.join("melete").join("sync_queue.db")
    }
...
fn worker_loop() {
    ...
    loop {
        ...
        loop {
            ...
            use melete_storage::remote_notebook_store::UpsertStrokeItem;
...
fn sync_inputs(
    ...
    (
        melete_core::Notebook,
        Vec<Section>,
        Vec<melete_core::Page>,
