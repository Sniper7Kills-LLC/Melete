...
const APP_ID: &str = "app.melete.Melete";
...
fn main() -> Result<()> {
    ...
    if let Err(e) = migrate_legacy_paths() {
        tracing::warn!("legacy path migration failed (continuing): {:#}", e);
    }
...
/* ────────────────────────────────────────────────────────────────────
   Melete — visual identity (paper-journal mood: deep indigo + amber)
   ──────────────────────────────────────────────────────────────────── */
...
/// One-shot rename of the pre-rebrand `journal/` data and config dirs onto
/// the new `melete/` paths. Runs once on startup; no-ops on fresh installs
/// and after the first successful migration.
fn migrate_legacy_paths() -> Result<()> {
...
fn build_ui(app: &adw::Application) -> Result<()> {
    ...
    let backend: Rc<RefCell<dyn NotebookBackend>> = Rc::new(RefCell::new(open_backend()?));
    ...
    // freshly-presented main window the very first time Melete launches
