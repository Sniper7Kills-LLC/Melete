...
fn rebuild_brush_picker(
    ...
    let mut entries: Vec<melete_core::Brush> = crate::brush_library::built_ins();
...
fn builtin_brush_for_tool(tool: Tool) -> Option<melete_core::Brush> {
        ...
        for color in palette {
            ...
            btn.set_child(Some(&swatch));
            {
                ...
                btn.connect_clicked(move |_| {
                    ...
                    {
                        let mut s = state_c.borrow_mut();
                        s.pen.color = melete_core::Color { r, g, b, a };
    ...
    for (idx, raw) in slots.iter().enumerate() {
        ...
        let on_change: Rc<dyn Fn()> = {
            ...
            Rc::new(move || {
                ...
                {
                    let mut s = state.borrow_mut();
                    s.pen.color = melete_core::Color { r, g, b, a: 255 };
                }
