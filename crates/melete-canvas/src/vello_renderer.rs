//! Vello renderer.
//!
//! Builds `vello::Scene` values from `melete_core::Stroke` data and
...
use melete_core::{BlendMode, Color as JColor, Point, Rect, Stroke};
use uuid::Uuid;
...
impl VelloRenderer {
    ...
    /// "select a page" message via `melete_widgets::WidgetRenderer::draw_placeholder`.
...
fn build_cursor_outline(px: f64, py: f64, radius: f64, ov: &OverlayState) -> BezPath {
    use melete_core::CursorShape as CS;
...
fn solid_color(color: melete_core::Color, alpha: f64) -> Color {
...
/// `StrokePoint`s.
fn build_smooth_path(pts: &[melete_core::StrokePoint]) -> BezPath {
    let n = pts.len();
    let mut path = BezPath::new();
    path.move_to((pts[0].x, pts[0].y));
    if n == 2 {
...
fn resample_path(pts: &[melete_core::StrokePoint], step: f64) -> Vec<(f64, f64, f64)> {
...
use melete_core::{Brush as RBrush, BrushLayer, ColorMod, Geometry, TipShape, WidthMode};
