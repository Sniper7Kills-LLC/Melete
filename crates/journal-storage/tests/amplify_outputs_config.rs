...
//!
//! These exercise the runtime override path (`JOURNAL_AMPLIFY_OUTPUTS`),
...
use journal_storage::remote_template_store::config::{load, ConfigError};
...
/// well-defined view of `JOURNAL_AMPLIFY_OUTPUTS`.
...
fn load_via_env_var_override() {
    ...
    std::env::set_var("JOURNAL_AMPLIFY_OUTPUTS", f.path());
    let out = load().expect("env-var override should parse");
    std::env::remove_var("JOURNAL_AMPLIFY_OUTPUTS");
