...
//!   1. `MELETE_AMPLIFY_OUTPUTS` env var (CI / explicit override).
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=MELETE_AMPLIFY_OUTPUTS");
    ...
    println!(
        "cargo:warning=amplify_outputs.json not found — built with empty stub. \
         ...
         MELETE_AMPLIFY_OUTPUTS=<path> for an explicit override."
    );
    println!(
...
fn resolve_source() -> Option<PathBuf> {
    if let Some(p) = std::env::var_os("MELETE_AMPLIFY_OUTPUTS") {
