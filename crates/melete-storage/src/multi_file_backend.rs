...
use crate::backend::{
    AssetBytes, AssetMeta, BrushRow, BrushStore, NotebookBackend, NotebookStore, PageStore,
...
impl NotebookBackend for MultiFileSqliteBackend {}

#[cfg(test)]
mod tests {
    use super::*;
    use chrono::Utc;
    use melete_core::{Notebook, NotebookKind, Page, PageId, Section, SectionId};
    use uuid::Uuid;
    ...
    fn delete_page_evicts_stroke_cache_for_notebook() {
        use melete_core::{BlendMode, Color, PenSettings, Rect, Stroke, StrokePoint, ToolStyle};
        let dir = tmpdir();
        let mut be = MultiFileSqliteBackend::open(dir.path()).unwrap();
        let nb = mk_notebook();
        be.insert_notebook(&nb).unwrap();
