...
//!
//! The journal app holds an `Rc<RefCell<dyn NotebookBackend>>` instead of a
...
pub trait StrokeStore {
    ...
    /// True when `id` is present locally but soft-deleted. Used by
    /// the pull path to skip re-merging a stroke the user erased.
...
/// One-stop trait for the storage layer. App holds `Rc<RefCell<dyn NotebookBackend>>`.
...
impl<T: NotebookBackend> PlannerQueries for T {}
