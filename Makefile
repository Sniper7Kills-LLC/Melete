...
DESKTOP       := resources/app.melete.Melete.desktop
ICON          := resources/icons/app.melete.Melete.svg
...
install:
	...
	install -d $(APP_DIR)
	sed 's|^Exec=melete-app|Exec=$(PREFIX)/bin/melete-app|' $(DESKTOP) > $(APP_DIR)/app.melete.Melete.desktop
	chmod 644 $(APP_DIR)/app.melete.Melete.desktop
	install -Dm644 $(ICON) $(ICON_DIR)/app.melete.Melete.svg
...
uninstall:
	...
	rm -f $(APP_DIR)/app.melete.Melete.desktop
	rm -f $(ICON_DIR)/app.melete.Melete.svg
	@if command -v update-desktop-database >/dev/null 2>&1; then \
		update-desktop-database $(APP_DIR); \
	fi
	@if command -v gtk-update-icon-cache >/dev/null 2>&1; then \
