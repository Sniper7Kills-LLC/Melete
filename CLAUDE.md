...
- **Brush Engine:** Composable layered model in `journal_core::brush` (`Brush` = `Vec<BrushLayer>` of `Geometry` × `WidthMode` × `TipShape` × `ColorMod` × `BlendMode` + a `CursorShape` and optional `default_color`). Renderer lowers `Brush` → Vello scene calls via `vello_renderer::draw_brush_into_scene`. Built-in tools (Pen / Pencil / Highlighter / Paintbrush / SprayCan / Calligraphy and their shape variants) are produced by `journal_canvas::built_in_brushes::legacy_brush_for(BrushStyle, &BrushParams)` so the legacy `BrushStyle`/`BrushParams` data continues to drive a unified composable code path; the per-style legacy `draw_*` fns are retired. Custom brushes live in `~/.config/journal/brushes.toml`; per-tool assignments live in `config.toml::tool_brush_assignments` (resolved by `Brush.id` against built-ins + library at boot). The full-screen Tool Editor (`crates/journal-app/src/tool_editor.rs`, opened via the hamburger "Tools…" entry, the toolbar's "Edit current tool…" entry, or the dev-mode popup's "Open in Tool Editor" link) lets users design brushes from scratch, including a draggable polygon editor for `TipShape::Custom` / `CursorShape::Custom`, an interactive Vello-rendered preview area, and the same renderer dispatch as the main canvas.
